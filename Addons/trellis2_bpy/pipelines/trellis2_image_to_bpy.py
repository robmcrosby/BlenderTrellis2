...
class Trellis2ImageToBpyPipeline(Pipeline):
    """
    Pipeline for inferring Trellis2 image-to-Blender 3D.
    ...
    def decode_latent(
        ...
        """
        print('decode shape...')
        task_queue.put({'stage':'Decoding Shape', 'step':step, 'steps':steps})
        step += 1
        meshes, subs = self.decode_shape_slat(shape_slat, resolution)
        ret = [np.concatenate([m.vertices.cpu().numpy() for m in meshes])]
        
        if tex_slat != None and not cancel_signal.is_set():
            print('decode texture...')
            task_queue.put({'stage':'Decoding Texture', 'step':step, 'steps':steps})
            tex_voxels = self.decode_tex_slat(tex_slat, subs)
            ret.append(np.concatenate([v.feats.cpu().numpy() for v in tex_voxels]))
        return ret
    ...
    def run(
        ...
        if cancel_signal.is_set():
            return []
        step = 1
        task_queue.put({'stage':'Sampling Sparse Structures', 'step':step, 'steps':steps})
        step += 1
        ...
        if pipeline_type == '512':
            if cancel_signal.is_set():
                return []
            task_queue.put({'stage':'Sampling Shape SLat', 'step':step, 'steps':steps})
            step += 1
            ...
            if gen_texture:
                if cancel_signal.is_set():
                    return []
                task_queue.put({'stage':'Sampling Texture SLat', 'step':step, 'steps':steps})
                step += 1
        ...
        if cancel_signal.is_set():
            return []
        return self.decode_latent(shape_slat, tex_slat, res, task_queue, cancel_signal, step, steps)
